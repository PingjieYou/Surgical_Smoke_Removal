...
def get_norm_layer(norm_type='instance'):
    ...
    Args:
        norm_type: 标准化层的名称：batch | instance | none
...
def define_G(opt, input_nc, output_nc, ngf, netG, norm=None, use_dropout=False, init_type='kaiming', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = SwinUnet(img_size=opt.crop_size, in_chans=input_nc, patch_size=opt.patch_size, window_size=opt.window_size, num_classes=output_nc)
    elif netG == 'pfanx':
        net = PFANX(input_nc=input_nc, output_nc=output_nc, ngf=ngf, hidden_dim=ngf, layers=[2, 2, 2], heads=[4, 4, 4], norm_layer_1=norm_layer)
    else:
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        Args:
            ...
            use_dropout: 是否使用dropout层
            n_blocks: ResNet块的数量
            padding_type: 卷积层中的填充层的名称：reflect | replicate | zero
        """
        assert (n_blocks >= 0)
...
class BlockV2(nn.Module):
    def __init__(self, dim, drop_path=0.):
        ...
        self.pwconv1 = nn.Linear(dim, 4 * dim)  # 点卷积/1x1卷积，使用线性层实现
        self.act = nn.GELU()
        self.grn = GRN(4 * dim)
...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, norm_layer=norm_layer, innermost=True)  # (b, in_c * 2, h, w)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None, submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        Args:
            outer_nc: 外部卷积层中的滤波器数量
            inner_nc: 内部卷积层中的滤波器数量
            input_nc: 输入图像/特征中的通道数
            submodule: 之前定义的子模块
            outermost: 如果这个模块是最外层模块
            innermost: 如果这个模块是最内层模块
            norm_layer: 标准化层
            use_dropout: 是否使用dropout层
        """
        ...
        elif innermost:  # 最内层模块
            upconv = nn.ConvTranspose2d(inner_nc, outer_nc,
                                        kernel_size=4, stride=2,
                                        padding=1, bias=use_bias)  # 上采样，尺寸变为原来的2倍
            down = [downrelu, downconv]  # 下采样，尺寸减半
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kernel_size, stride=2, padding=padding_size), nn.LeakyReLU(0.2, True)]
        nf_mult = 1
        nf_mult_prev = 1
        for n in range(1, n_layers):
            ...
            nf_mult = min(2 ** n, 8)
            sequence += [
                nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kernel_size, stride=2, padding=padding_size, bias=use_bias),
                norm_layer(ndf * nf_mult),
                nn.LeakyReLU(0.2, True)
            ]
        ...
        nf_mult = min(2 ** n_layers, 8)
        sequence += [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kernel_size, stride=1, padding=padding_size, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ]

        sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kernel_size, stride=1, padding=padding_size)]  # output 1 channel prediction map
